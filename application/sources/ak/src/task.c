...
#if defined(AK_TASK_OBJ_LOG_ENABLE)
	log_queue_init(&log_task_dbg_object_queue, (uint32_t)task_dbg_active_obj_queue, (LOG_QUEUE_OBJECT_SIZE / sizeof(ak_msg_t)), sizeof(ak_msg_t), mem_write, mem_read);
#endif

	/* init irq log queue */
#if defined(AK_IRQ_OBJ_LOG_ENABLE)
	log_queue_init(&log_irq_queue, (uint32_t)irq_queue, (LOG_QUEUE_IRQ_SIZE / sizeof(exception_info_t)), sizeof(exception_info_t), mem_write, mem_read);
#endif
		...
		if (current_active_object.des_task_id > 0) {
...
#if defined(AK_TASK_LOG_CONSOLE_ENABLE)
			{
				...
				xprintf("taskID: %d\tmsgType:0x%x\trefCnt:%d\tsig:%d\t\twaitTime:%d\texeTime:%d\n", current_active_object.des_task_id,
						(current_active_object.ref_count & AK_MSG_TYPE_MASK), (current_active_object.ref_count & AK_MSG_REF_COUNT_MASK), current_active_object.sig, (wait_time),
						(exe_time));
			}
#endif
		}
	...
	for (uint8_t pri = 1; pri <= TASK_PRI_MAX_SIZE; pri++) {
		...
		if (t_task_ready & t_tcb->mask) {
			...
			while (t_msg != AK_MSG_NULL) {
				/* dump message queue */
				xprintf("srcTaskID:%d\tdesTaskID:%d\tmsgType:0x%x\trefCnt:%d\tsig:%d\n", t_msg->src_task_id, t_msg->des_task_id, (t_msg->ref_count & AK_MSG_TYPE_MASK),
