...
#define OB_BOOT_BANK2                                                                      \
	((uint8_t)0x00) /*!< At startup, if boot pins are set in boot from user Flash position \
						 and this parameter is selected the device will boot from Bank 2   \
						 or Bank 1, depending on the activation of the bank */
#define OB_BOOT_BANK1                                                                      \
	((uint8_t)0x80) /*!< At startup, if boot pins are set in boot from user Flash position \
...
#define OB_BOR_OFF                                                                                           \
	((uint8_t)0x00)					  /*!< BOR is disabled at power down, the reset is asserted when the VDD \
...
#define IS_OB_BOR_LEVEL(LEVEL) \
	(((LEVEL) == OB_BOR_OFF) || ((LEVEL) == OB_BOR_LEVEL1) || ((LEVEL) == OB_BOR_LEVEL2) || ((LEVEL) == OB_BOR_LEVEL3) || ((LEVEL) == OB_BOR_LEVEL4) || ((LEVEL) == OB_BOR_LEVEL5))
...
#define FLASH_FLAG_RDERR                                \
	FLASH_SR_RDERR /*!< FLASH Read protected error flag \
...
#define IS_FLASH_GET_FLAG(FLAG)                                                                                                                                   \
	(((FLAG) == FLASH_FLAG_BSY) || ((FLAG) == FLASH_FLAG_EOP) || ((FLAG) == FLASH_FLAG_ENDHV) || ((FLAG) == FLASH_FLAG_READY) || ((FLAG) == FLASH_FLAG_WRPERR) || \
	 ((FLAG) == FLASH_FLAG_PGAERR) || ((FLAG) == FLASH_FLAG_SIZERR) || ((FLAG) == FLASH_FLAG_OPTVERR) || ((FLAG) == FLASH_FLAG_OPTVERRUSR) || ((FLAG) == FLASH_FLAG_RDERR))
/**
 * @}
 */
...
#define FLASH_PDKEY2                                                          \
	((uint32_t)0xFAFBFCFD) /*!< Flash power down key2: used with FLASH_PDKEY1 \
							   to unlock the RUN_PD bit in FLASH_ACR */

#define FLASH_PEKEY1 ((uint32_t)0x89ABCDEF) /*!< Flash program erase key1 */
#define FLASH_PEKEY2                                                                      \
	((uint32_t)0x02030405) /*!< Flash program erase key: used with FLASH_PEKEY2           \
								to unlock the write access to the FLASH_PECR register and \
								data EEPROM */

#define FLASH_PRGKEY1 ((uint32_t)0x8C9DAEBF) /*!< Flash program memory key1 */
#define FLASH_PRGKEY2                                                              \
	((uint32_t)0x13141516) /*!< Flash program memory key2: used with FLASH_PRGKEY2 \
								to unlock the program memory */

#define FLASH_OPTKEY1 ((uint32_t)0xFBEAD9C8) /*!< Flash option key1 */
#define FLASH_OPTKEY2                                                         \
	((uint32_t)0x24252627) /*!< Flash option key2: used with FLASH_OPTKEY1 to \
