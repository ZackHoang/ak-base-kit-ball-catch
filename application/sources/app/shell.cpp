...
int32_t shell_ver(uint8_t *argv) {
	...
	LOGIN_PRINT("App version: %d.%d.%d.%d\n", app_info.version[0],
				app_info.version[1], app_info.version[2], app_info.version[3]);
	...
	switch (*(argv + 4)) {
		default:
			...
			while (lgn_cmd_table[idx].cmd != (const int8_t *)0) {
				LOGIN_PRINT("%s\t-> %s\n\n", lgn_cmd_table[idx].cmd,
							lgn_cmd_table[idx].info);
				idx++;
			}
			break;
	}
	...
	switch (*(argv + 6)) {
		...
		case 'r':
			...
			flash_write(APP_FLASH_AK_DBG_FATAL_LOG_SECTOR,
						(uint8_t *)&login_fatal_log, sizeof(fatal_log_t));
		...
		case 'l': {
			...
			LOGIN_PRINT(
				...
				login_fatal_log.current_active_object.dbg_handler.start_exe -
					login_fatal_log.current_active_object.dbg_handler
						.start_post);
		...
		case 'm': {
			...
			for (uint32_t index = 0;
				 index < (LOG_QUEUE_OBJECT_SIZE / sizeof(ak_msg_t)); index++) {
				/* reset watchdog */
				sys_ctrl_independent_watchdog_reset();
				sys_ctrl_soft_watchdog_reset();

				flash_read(flash_sys_log_address, (uint8_t *)&t_msg,
						   sizeof(ak_msg_t));
				flash_sys_log_address += sizeof(ak_msg_t);

				uint32_t wait_time;
				(void)wait_time;
				if (t_msg.dbg_handler.start_exe >=
					t_msg.dbg_handler.start_post) {
					wait_time = t_msg.dbg_handler.start_exe -
								t_msg.dbg_handler.start_post;
				}
				else {
					wait_time = t_msg.dbg_handler.start_exe +
								(0xFFFFFFFF - t_msg.dbg_handler.start_post);
				}

				uint32_t exe_time;
				(void)exe_time;
				if (t_msg.dbg_handler.stop_exe >= t_msg.dbg_handler.start_exe) {
					exe_time = t_msg.dbg_handler.stop_exe -
							   t_msg.dbg_handler.start_exe;
				}
				else {
					exe_time = t_msg.dbg_handler.stop_exe +
							   (0xFFFFFFFF - t_msg.dbg_handler.start_exe);
				}

				sys_ctrl_delay_ms(5);

				LOGIN_PRINT(
					"index: %d\ttask_id: %d\tmsg_type:0x%x\tref_count:%d\tsig: "
					"%d\t\twait_time: %d\texe_time: %d\n",
					index, t_msg.des_task_id,
					(t_msg.ref_count & AK_MSG_TYPE_MASK),
					(t_msg.ref_count & AK_MSG_REF_COUNT_MASK), t_msg.sig,
					(wait_time), (exe_time));
			}
		} break;
		...
		case 'e': {
			...
			for (uint32_t index = 0;
				 index < (LOG_QUEUE_IRQ_SIZE / sizeof(exception_info_t));
				 index++) {
				/* reset watchdog */
				sys_ctrl_independent_watchdog_reset();
				sys_ctrl_soft_watchdog_reset();

				flash_read(flash_irq_log_address, (uint8_t *)&t_exception_info,
						   sizeof(exception_info_t));
				flash_irq_log_address += sizeof(exception_info_t);

				LOGIN_PRINT(
					"index: %d\texcept_number: %d\tirq_number: %d\ttimestamp: "
					"%d\n",
					index, t_exception_info.except_number,
					(int32_t)((int32_t)t_exception_info.except_number -
							  (int32_t)
								  SYS_IRQ_EXCEPTION_NUMBER_IRQ0_NUMBER_RESPECTIVE),
		...
		case 'R': {
			...
			if (len == 3) {
				...
				uint8_t val;
				uint32_t len_of_ram =
					(uint32_t)&_estack - (uint32_t)&_start_ram;

				uint32_t colum =
					(uint32_t)strtol(str_parser_get_attr(2), NULL, 0);
				...
				for (uint32_t i = 0; i < len_of_ram; i++) {
					...
					flash_read(APP_FLASH_DUMP_RAM_START_ADDR + i, &val,
							   sizeof(uint8_t));
					LOGIN_PRINT("%d\t", val);
				}
				LOGIN_PRINT("\n");
...
int32_t shell_eps(uint8_t *argv) {
	...
	switch (*(argv + 4)) {
		...
		case 'r': {
			LOGIN_PRINT("erasing...\n");
			eeprom_erase(EEPROM_START_ADDR,
						 EEPROM_END_ADDR - EEPROM_START_ADDR);
	...
	switch (*(argv + 4)) {
		case 'r': {
			if (len == 4) {
				...
				start_addr =
					(uint8_t *)((uint32_t)strtol(str_start_addr, NULL, 0));
				stop_addr =
					(uint8_t *)((uint32_t)strtol(str_stop_addr, NULL, 0));

				LOGIN_PRINT("start_addr: 0x%x\n", start_addr);
				LOGIN_PRINT("stop_addr: 0x%x\n", stop_addr);

				if ((uint32_t)start_addr >= ((uint32_t)&_start_ram) &&
					(uint32_t)stop_addr <= ((uint32_t)&_estack)) {
					/* start dump ram */
					LOGIN_PRINT("\n");

					for (uint8_t *addr_index = start_addr;
						 addr_index < stop_addr; addr_index++) {
...
int32_t shell_boot(uint8_t *argv) {
	switch (*(argv + 5)) {
		case 'i': {
			...
			LOGIN_PRINT(
				"[b_cmd] cmd: %d container:%d io_driver: %d des_addr: 0x%08X "
				"src_addr: 0x%08X\n",
				sys_boot.fw_boot_cmd.cmd, sys_boot.fw_boot_cmd.container,
				sys_boot.fw_boot_cmd.io_driver, sys_boot.fw_boot_cmd.des_addr,
			...
			LOGIN_PRINT("[a_cmd] cmd: %d container:%d io_driver: %d des_addr: "
						"0x%08X src_addr: 0x%08X\n",
						sys_boot.fw_app_cmd.cmd, sys_boot.fw_app_cmd.container,
...
int32_t shell_buzzer(uint8_t *argv) {
	switch (*(argv + 5)) {
		...
		default:
			LOGIN_PRINT("\n[HELP]\n");
			LOGIN_PRINT("1. \"beep i\"                           : init buzzer "
						"play tones \n");
			LOGIN_PRINT("2. \"beep 1\"                           : buzzer play "
						"tones startup \n");
			LOGIN_PRINT("3. \"beep 2\"                           : buzzer play "
						"tones three beeps \n");
			LOGIN_PRINT("4. \"beep 3\"                           : buzzer play "
						"tones super mario bros \n");
			LOGIN_PRINT("4. \"beep 4\"                           : buzzer play "
						"tones merry chrismast \n");
			LOGIN_PRINT("5. \nCustom\n");
			break;
	}
	...
	switch (*(argv + 7)) {
		case 'r':
			...
			for (uint16_t i = 0; i < MB_ES35SW_TH_Sensor.listRegAmount; i++) {
				LOGIN_PRINT("regAddr[%d]: %d\t", i + 1,
							MB_ES35SW_TH_Sensor.listRegDevice[i].regAddress);
			...
			for (uint16_t i = 0; i < MB_LHIO404_IO_Device.listRegAmount; i++) {
				LOGIN_PRINT("regAddr[%d]: %d\t", i + 1,
							MB_LHIO404_IO_Device.listRegDevice[i].regAddress);
				LOGIN_PRINT("\trawValue: %d \n",
