...
void view_scr_lhio404_io_device() {
	...
	for (uint16_t regIndex = 4; regIndex < MB_LHIO404_IO_Device.listRegAmount;
		 ++regIndex) {
...
void scr_lhio404_io_device_handle(ak_msg_t *msg) {
	switch (msg->sig) {
		case SCREEN_ENTRY: {
			...
			timer_set(AC_TASK_DISPLAY_ID, AC_DISPLAY_SHOW_MODBUS_PULL_UPDATE,
					  AC_DISPLAY_SHOW_MODBUS_PULL_INTERVAL, TIMER_PERIODIC);

			timer_set(AC_TASK_DISPLAY_ID, AC_DISPLAY_SHOW_MODBUS_PULL_SLEEP,
