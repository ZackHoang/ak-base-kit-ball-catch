...
void is_touching_bar(ball_t &ball) {
	if (ball.y + BALL_RADIUS >= game_data.bar.y - BAR_HEIGHT && ball.y - BALL_RADIUS <= game_data.bar.y + BAR_HEIGHT && ball.x >= game_data.bar.x &&
		ball.x <= game_data.bar.x + BAR_WIDTH && game_data.game_over == false) {
		BUZZER_PlayTones(tones_bang);
		ball.y_speed = -ball.y_speed;
		game_data.score++;
	}
